...
// ADR-002 échange final : la sync des STRUCTURES / EMPLOIS employeuses dans coop a été retirée
// (l'employeuse CN vit dans `main`, source de vérité, non répliquée dans coop). Cette suite ne
// couvre donc plus que le comportement SURVIVANT de l'orchestrateur : no-op sur réponse null,
// transitions du flag conseiller numérique (marqueurs Brevo), coordinateur (create, never-delete,
// hors-dispositif), et l'invariant « les lieux d'activité ne sont pas synchronisés ici ».
...
describe('updateUserFromDataspaceData', () => {
  ...
  describe('Was not Conum, becomes Conum', () => {
    ...
    test('should set isConseillerNumerique=true and mark the transition', async () => {
      const user = await prismaClient.user.findUniqueOrThrow({
        where: { id: mediateurSansActivites.id },
      ...
      // ADR-002 échange final : plus de sync des structures dans coop.
      expect(result.changes.structuresSynced).toBe(0)
  ...
  describe('Was Conum, no longer is', () => {
    ...
    test('should set isConseillerNumerique=false and mark the transition', async () => {
      const user = await prismaClient.user.findUniqueOrThrow({
