...
export const BeneficiaireAnonymeFields = withForm({
  ...
  render: ({
    ...
    const [communeFieldLabel, setCommuneFieldLabel] = useState<string>('')

    useEffect(() => {
      if (form.getFieldValue('beneficiaire.communeResidence') == null) return
      setCommuneFieldLabel(lieuCommuneData?.label ?? '')
    }, [lieuCommuneData, form])
              ...
              {({ getLabelProps, getInputProps, ...options }) => (
                <div className="fr-mb-12v">
                  <field.Input
                    isPending={isPending}
                    isConnected={false}
                    value={communeFieldLabel}
                    setValue={setCommuneFieldLabel}
