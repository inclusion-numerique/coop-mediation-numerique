...
// Re-canonicalise une fiche via le transfer layer. `modification` est réémis
// pour que l'extension timestamp ne bumpe pas (un nettoyage de données n'est pas
// une édition). Une donnée invalide fait throw toDomain → la fiche est remontée.
const recanonicalise = (row: BeneficiaireRow) => {
  try {
    const { id: _id, ...rest } = beneficiaireFromDomain(
      beneficiaireToDomain(row),
    )
