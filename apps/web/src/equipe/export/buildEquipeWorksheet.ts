...
const formatFilters = (filters: EquipeSearchParams): string => {
  const parts: string[] = []

  if (filters.statut) {
    const statuts = filters.statut.split(',').filter(Boolean) as FilterParam[]
    parts.push(...statuts.map((s) => statutFilterLabels[s] ?? s))
  }

  if (filters.role) {
    parts.push(roleFilterLabels[filters.role] ?? filters.role)
  }
