...
const computeStatut = ({
  ...
  if (type === 'invited') {
    return `Invitation envoyée le ${formatDate(creation)}`
  }

  if (userDeleted) {
    return `Profil supprimé le ${formatDate(userDeleted)}`
  }

  if (suppression) {
    return `Ancien membre depuis le ${formatDate(suppression)}`
  }
  ...
  if (dateDerniereActivite < twoMonthsAgo) {
...
export const getEquipeExportData = async ({
  ...
  // Utilise la même requête que le tableau pour garantir la cohérence
  const mediateurs = await findAllMediateursCoordonneBy({ id: coordinateurId })(
    searchParams,
  )

  // Récupérer les infos de contrat pour les conseillers numériques
