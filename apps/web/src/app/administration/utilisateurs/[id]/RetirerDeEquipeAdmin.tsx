...
const RetirerDeEquipeAdminComponent = ({
  mediateurId,
  coordinateurId,
  displayName,
}: {
  ...
  const onRemoveFromTeam = async () => {
    try {
      await removeFromTeamMutation.mutateAsync({ mediateurId, coordinateurId })
      closeRemoveFromTeamModal()
      router.refresh()
      createToast({
        priority: 'success',
        message: `${displayName} a bien été retiré de l'équipe`,
      })
    } catch {
      closeRemoveFromTeamModal()
      createToast({
        priority: 'error',
        message: `Une erreur est survenue lors de la suppression de ${displayName} de l'équipe, veuillez réessayer ultérieurement.`,
      })
    }
  }
  ...
  return (
    <>
      {isMounted &&
        createPortal(
          <RemoveFromTeamModal
            title={`Retirer '${displayName}' de l'équipe`}
            buttons={[
              {
                children: 'Annuler',
                priority: 'secondary',
            ...
            <Notice
              ...
              title={
                <span className="fr-text--left fr-text-default--grey fr-text--regular">
                  Le coordinateur n'aura plus accès aux prochaines activités et
                  statistiques enregistrés{' '}
                  <b>
                    à partir du moment où '{displayName}' sera retiré de
                    l'équipe.
                  </b>
                  <br />
                  <br />
                  Il conservera uniquement l'accès aux activités et statistiques
                  archivés <b>jusqu'à ce jour.</b>
                </span>
              }
            />
          </RemoveFromTeamModal>,
...
export const RetirerDeEquipeAdmin = withTrpc(RetirerDeEquipeAdminComponent)
